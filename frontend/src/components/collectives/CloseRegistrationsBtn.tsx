...
const CloseRegistrationsBtn = (props: IStartCollectiveBtn) => {
    ...
    return (
        <Button onClick={close} size="sm" radius={'md'} rightSection={loading ? <Loader size={'sm'} c={'white'} /> : <IconRepeat stroke={1.5} />} variant="filled">{loading ? "Close Registrations" : "Closing Registrations"}</Button>
    )
}
